...
class DataProcessThread(QThread):
    ...
    def detect_data_format(folder_path):
        ...
        if path.is_file() and loading.tofwerk_loading.is_tofwerk_file(path):
            return "tofwerk"
        
        if path.is_dir():
            h5_files = list(path.glob("*.h5"))
            if h5_files and any(loading.tofwerk_loading.is_tofwerk_file(f) for f in h5_files):
                return "tofwerk"
    ...
    def process_nu_data(self):
        ...
        selected_data = loading.vitesse_loading.select_nu_signals(
            masses=masses,
            signals=signals,
