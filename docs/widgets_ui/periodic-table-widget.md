...
| Name | Value |
|------|-------|
| `PRESET_LISTS` | `{'71A': ['Ag', 'Al', 'As', 'B', 'Ba', 'Be', 'Ca', 'Cd', '…` |
...
| `update_selection` | `(self, selections_by_element: dict[str, list[float]]) → tuple[int, lis` | Updates the buttons with a batch selection of elements by masses. |
