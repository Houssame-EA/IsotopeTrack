...
|--------|-----------|-------------|
| `__init__` | `(self, cfg, input_data, parent=None, scope='all')` | Preserved behavior: |
| `_build_ui` | `(self)` | Build settings widgets for the selected scope. |
| `_try_apply` | `(self)` |  |
| `_try_done` | `(self)` |  |
| `_pick_shade_color` | `(self)` |  |
| `_on_shade_type_changed` | `(self, text)` |  |
| `_move_up` | `(self)` |  |
| `_move_down` | `(self)` |  |
...
|----------|-----------|-------------|
| `_y_label` | `(cfg)` |  |
| `_element_color` | `(element, index, cfg)` |  |
| `_fmt_elem` | `(elem, cfg)` |  |
| `_is_multi` | `(input_data)` |  |
| `_available_elements` | `(input_data)` |  |
| `_filter_values` | `(values, data_type, log_y, cfg=None)` |  |
...
| `_tag_box_color_identity` | `(item, cfg)` | Tag a box rect with its colour identity so double-click edits persist. |
| `_draw_box` | `(plot_item, x, values, color, alpha, width, cfg)` |  |
| `_draw_violin` | `(plot_item, x, values, color, alpha, width, cfg)` |  |
| `_draw_box_violin` | `(plot_item, x, values, color, alpha, width, cfg)` |  |
| `_draw_strip` | `(plot_item, x, values, color, alpha, width, cfg)` |  |
| `_draw_half_violin_box` | `(plot_item, x, values, color, alpha, width, cfg)` |  |
| `_draw_notched_box` | `(plot_item, x, values, color, alpha, width, cfg)` |  |
| `_draw_bar_errors` | `(plot_item, x, values, color, alpha, width, cfg)` |  |
