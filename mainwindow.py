...
class MainWindow(QMainWindow):
    ...
    def on_sample_selected(self, item):
        ...
        if sample_name in self.data_by_sample:
            ...
            if hasattr(self, 'info_tooltip') and self.info_tooltip.isVisible():
                ...
                total_elements = (sum(len(isotopes)
                                      for isotopes in self.selected_isotopes.values())
                                  if self.selected_isotopes else 0)
                accuracy = self.calculate_accuracy()
        ...
        try:
            ...
            if removed_isotopes:
                ...
                for sample_name in self.data_by_sample:
                    ...
                    if sample_name in self.sample_detected_peaks:
                        peaks_to_remove = [(e, i)
                                           for e, i in removed_isotopes
                                           if (e, i) in self.sample_detected_peaks[sample_name]]
    ...
    def update_parameters_table(self, force=True):
        ...
        try:
            signature = (
                self.current_sample,
                tuple(
                    sorted(self.selected_isotopes_as_element_mass())
                ),
            ...
            with self.parameters_table._model.bulk_update():
                for row in range(self.parameters_table.rowCount()):
                    ...
                    if per_isotope_mode:
                        ...
                        for element, isotope in self.selected_isotopes_as_element_mass():
                            # Find the element key that gives the display_label to use it in the sample_parameters
                            element_key = f"{element}-{isotope:.4f}"
                            if self.get_formatted_label(element_key) == display_label:
                                el_params = (self.sample_parameters
    ...
    def highlight_multi_element_particle(self):
                ...
                for isotope in isotopes:
                    ...
                    if self.get_formatted_label(element_key) == display_label:
                        ...
                        if closest_mass in self.data:
                            ...
                            try:
                                if ((element, isotope) in self.detected_peaks
                                        and self.current_sample in self.element_thresholds
                                        and element_key in self.element_thresholds[self.current_sample]):
        ...
        if element_data:
            ...
            info_y = (y_min + 0.6 * (y_max - y_min)
                      if 'y_max' in locals() and 'y_min' in locals()
                      else min_signal + 0.6 * (max_signal - min_signal))
    ...
    def update_calibration_display(self):
        ...
        for _, display_label, element_key in element_data:
            ...
            display_text += (f"<td>{lod_counts:.2f}</td>"
                             if isinstance(lod_counts, (int, float))
                             else "<td>Not calculated</td>")
